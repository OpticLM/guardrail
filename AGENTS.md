...
**`guardrail-napi` (Node bindings).** One `spawn(command, args?, options?)` function plus a reusable `Sandbox`; `options` mirrors `SandboxConfig` fields (camelCase, see `SpawnOptions`). The backend comes from the Rust `guardrail::PlatformBackend` facade — each published binary targets exactly one OS. stdin/stdout/stderr each accept per-spawn `'inherit'` (default) | `'pipe'` | `'ignore'`. The native layer exposes promise primitives (`wait`, `readStdout`/`readStderr`, `writeStdin`/`endStdin`), each serviced by a dedicated OS thread — never the libuv threadpool, which idle persistent children would starve. The hand-written `index.js`/`index.d.ts` wrap them into the public classes whose piped stdio are real Node `Readable`/`Writable` streams and whose `wait()` (status-only) is memoized; `napi build` emits the generated loader as `binding.js`/`binding.d.ts` (`--js`/`--dts` flags).
