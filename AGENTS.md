...
**Platform backends.** Each implements `Backend` and applies confinement. Linux applies confinement inside `std::process::Command::pre_exec` (after fork, before exec). macOS keeps `pre_exec` async-signal-safe, then execs `/usr/bin/sandbox-exec` so Seatbelt profile parsing happens in a fresh single-threaded launcher. Windows launches the child suspended, installs policy (Job Object + AppContainer + ACL), then resumes.

- `guardrail-linux` — Landlock (fs, network, IPC domains), seccomp-BPF (network + kernel attack surface), mandatory per-spawn IPC/user/mount namespaces, `setrlimit` (resources), `NO_NEW_PRIVS` first. Seccomp is applied **last** so its filter doesn't interfere with Landlock's setup syscalls. Deny rules beneath allowed parents are enforced with mount masks inside the per-spawn user + mount namespace (see `guardrail-linux/src/ns.rs`), entered in `pre_exec` before Landlock — so the ordered policy stays live for files created after backend construction.
- `guardrail-macos` — Seatbelt (`/usr/bin/sandbox-exec`) + `setrlimit`. Profile generation and launcher argv/envp construction happen in the parent; the post-fork child performs only descriptor cleanup, rlimits, and `execve` into the launcher. The crate is native-only and tested on macOS. Seatbelt uses `(deny default)`, so a real binary often needs runtime grants (dyld paths, sysctl names) beyond the portable policy; the `darwin_sandbox_profiles` escape hatch + the crate-level tips doc are the answer — see `guardrail-macos/src/lib.rs`.
- `guardrail-windows` — AppContainer (per-run SID, fs/network), Job Object (process tree dies with the sandbox), ACL grants are *additive* (only ever append ACEs). Standard handles are inherited from an inert suspended helper process, keeping duplicable host handles non-inheritable; real console handles use a launch-specific helper because Windows only permits sharing them during process creation. Process inspectors therefore report a helper as the sandbox child's immediate parent. AppContainer baseline IPC isolation applies independently (see the crate docs for its access checks and limits).

**`guardrail-napi` (Node bindings).** One `spawn(command, args?, options?)` function plus a reusable `Sandbox`; `options` mirrors `SandboxConfig` fields (camelCase, see `SpawnOptions`). The backend comes from the Rust `guardrail::PlatformBackend` facade — each native binary targets exactly one OS, and all six supported binaries ship in the single `@opticlm/guardrail` package. stdin/stdout/stderr each accept per-spawn `'inherit'` (default) | `'pipe'` | `'ignore'`. The native layer exposes promise primitives (`wait`, `readStdout`/`readStderr`, `writeStdin`/`endStdin`), each serviced by a dedicated OS thread — never the libuv threadpool, which idle persistent children would starve. `napi build` generates `binding.mjs`, `binding.cjs`, and `binding.d.ts`; the single hand-written `index.ts` facade re-exports that surface while wrapping the child primitives into real Node `Readable`/`Writable` streams and memoizing status-only `wait()`. TypeScript checks the facade against `binding.d.ts`, then tsdown bundles each generated loader with it into the published `index.mjs`, `index.cjs`, and `index.d.ts`. The npm package contains those final files and the six native `.node` binaries.
