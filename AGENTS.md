...
- `guardrail-linux` — Landlock (fs), seccomp-BPF (network + IPC), `setrlimit` (resources), `NO_NEW_PRIVS` first. Seccomp is applied **last** so its filter doesn't interfere with Landlock's setup syscalls. Deny rules beneath allowed parents are enforced with mount masks inside a per-spawn unprivileged user + mount namespace (see `guardrail-linux/src/ns.rs`), entered in `pre_exec` before Landlock — so the ordered policy stays live for files created after backend construction. The only backend that consumes `SandboxConfig::linux_ipc`.
